...
def save_dataset_as_jsonl(
    ...
    for split in dataset:
        ...
        def _update_example(example: dict):
            example["text"] = " ".join([m['content'] for m in example[text_field]])
            example["message"] = example[text_field]
            # example.pop(text_field)
            # print(example)
            return example
...
if __name__ == "__main__":
    ...
    parser.add_argument("--text-field", type=str, required=False, default="messages")
